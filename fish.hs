...
specGuide:: Handle->IO()
specGuide specfile= 
    do putStrLn "请根据向导完成规约描述，若所有规约已输入完成，请输入done"
       putStrLn "(a)要求控制对象在整个区间内持续成立 (b)要求控制对象在整个区间内持续不成立 (c)要求控制对象在区间左端点成立，其余时刻均不成立 （done）规约输入完成"
       ans<-getLine
       let info_prop=ans in
        case ans of 
            "done"->do {putStrLn "规约文件spec.txt生成完成"   return()} 
            _ -> do putStrLn "1.选择对控制对象成立范围的要求（a）无条件 （b）在左参照点成立后 （c）在两个参照点之间 (d)与参照点同时成立"
                    ans1<-getLine
                    case ans1 of
                        "a"-> do putStrLn "请输入控制对象名称"
                                 info_p<-getLine
